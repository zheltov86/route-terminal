...
def api_export_organization():
    ...
    L.append(indent(1) + '<msg:Header>')
    L.append(indent(2) + '<msg:Format>http://v8.1c.ru/edi/edi_stnd/EnterpriseData/1.8</msg:Format>')
    L.append(indent(2) + '<msg:CreationDate>' + now + '</msg:CreationDate>')
    L.append(indent(2) + '<msg:AvailableVersion>1.8</msg:AvailableVersion>')
    L.append(indent(1) + '</msg:Header>')
    L.append(indent(1) + '<Body xmlns="http://v8.1c.ru/edi/edi_stnd/EnterpriseData/1.8">')
    L.append(indent(2) + '<Справочник.Валюты>')
    L.append(indent(3) + '<КлючевыеСвойства>')
    L.append(indent(4) + '<Ссылка>7e3e0ef4-8295-11f1-8af8-04ecd881cf53</Ссылка>')
    L.append(indent(4) + '<ДанныеКлассификатора><Код>643</Код><Наименование>руб.</Наименование></ДанныеКлассификатора>')
    L.append(indent(3) + '</КлючевыеСвойства>')
    L.append(indent(3) + '<ПараметрыПрописи>рубль, рубля, рублей, м, копейка, копейки, копеек, ж, 2 знака</ПараметрыПрописи>')
    L.append(indent(2) + '</Справочник.Валюты>')
    L.append(indent(2) + '<Справочник.Организации>')
    L.append(indent(3) + '<КлючевыеСвойства>')
    L.append(indent(4) + '<Ссылка>a1b2c3d4-e5f6-7890-abcd-ef1234567890</Ссылка>')
    L.append(indent(4) + '<Наименование>ООО ЦентрСнабжение</Наименование>')
    L.append(indent(4) + '<НаименованиеСокращенное>ООО ЦентрСнабжение</НаименованиеСокращенное>')
    L.append(indent(4) + '<НаименованиеПолное>Общество с ограниченной ответственностью ЦентрСнабжение</НаименованиеПолное>')
    L.append(indent(4) + '<ИНН>7708123456</ИНН>')
    L.append(indent(4) + '<КПП>770801001</КПП>')
    L.append(indent(4) + '<ЮридическоеФизическоеЛицо>ЮридическоеЛицо</ЮридическоеФизическоеЛицо>')
    L.append(indent(3) + '</КлючевыеСвойства>')
    L.append(indent(3) + '<Префикс>ЦС</Префикс>')
    L.append(indent(3) + '<КодОКВЭД>46.46</КодОКВЭД>')
    L.append(indent(3) + '<ОГРН>1237700123456</ОГРН>')
    L.append(indent(3) + '<ЮридическийАдрес>г. Москва, ул. Снабженческая, д. 12, стр. 3</ЮридическийАдрес>')
    L.append(indent(3) + '<Телефон>+7 (495) 123-45-67</Телефон>')
    L.append(indent(3) + '<Email>info@centrsnab.ru</Email>')
    L.append(indent(3) + '<Руководитель>Иванов Сергей Александрович</Руководитель>')
    L.append(indent(3) + '<ГлавныйБухгалтер>Петрова Мария Ивановна</ГлавныйБухгалтер>')
    L.append(indent(3) + '<Банк>ПАО Сбербанк</Банк>')
    L.append(indent(3) + '<БИК>044525225</БИК>')
    L.append(indent(3) + '<РасчетныйСчет>40702810400000012345</РасчетныйСчет>')
    L.append(indent(3) + '<КорреспондентскийСчет>30101810400000000225</КорреспондентскийСчет>')
    L.append(indent(2) + '</Справочник.Организации>')
    L.append(indent(1) + '</Body>')
    L.append('</Message>')
